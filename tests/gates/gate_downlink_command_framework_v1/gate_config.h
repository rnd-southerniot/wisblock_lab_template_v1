/**
 * @file gate_config.h
 * @brief Gate 8 — Downlink Command Framework v1 Configuration
 * @version 1.0
 * @date 2026-02-25
 *
 * Validates bidirectional communication: receive ChirpStack downlink,
 * parse binary command, apply safe runtime change (poll interval or
 * slave ID), confirm with status ACK uplink.
 *
 * Single shared config for both platforms via #ifdef CORE_RAK4631.
 * LoRaWAN/Modbus parameters reuse Gate 5-7 values.
 * Modbus RTU and CRC constants redefined so shared .cpp files compile.
 */
 ...
 * Gate Identity
 * ============================================================ */
#define GATE_NAME                       "gate_downlink_command_framework_v1"
...
/* ============================================================
 * Gate 8 — Downlink-Specific Timing
 * ============================================================ */
#define GATE8_JOIN_TIMEOUT_MS           30000   /* OTAA join timeout */
#define GATE8_DOWNLINK_WAIT_MS          60000   /* Wait up to 60s for downlink */
#define GATE8_INITIAL_UPLINKS           2       /* Send 2 uplinks to open RX windows */
...
#define GATE_FAIL_UPLINK                3   /* Initial uplinks failed */
#define GATE_FAIL_PARSE                 4   /* Downlink parse error */
#define GATE_FAIL_APPLY                 5   /* Downlink command apply error */
