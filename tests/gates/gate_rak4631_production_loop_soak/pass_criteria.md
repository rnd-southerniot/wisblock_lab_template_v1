# Gate 7 — RAK4631 Production Loop Soak: Pass Criteria

## Gate Identity
- **Name:** `gate_rak4631_production_loop_soak`
- **ID:** 7
- **Version:** 1.0
- **Hardware:** RAK4631 (nRF52840 + SX1262) on RAK19007
...
The scheduler runs naturally via `tick()` — no `fireNow()` intervention.
...
2. **Full Duration** — System runs for the entire `GATE7_SOAK_DURATION_MS`
   without crash, reboot, or watchdog reset.

3. **Minimum Uplinks** — At least `GATE7_MIN_UPLINKS` (3) successful
   uplinks sent via LoRaWAN transport.

4. **Modbus Stability** — No more than `GATE7_MAX_CONSEC_FAILS` (2)
   consecutive Modbus read failures.

5. **Uplink Stability** — No more than `GATE7_MAX_CONSEC_FAILS` (2)
   consecutive uplink send failures.

6. **Cycle Duration** — Every cycle completes within `GATE7_MAX_CYCLE_DURATION_MS`
...
| 2 | `GATE_FAIL_INSUFFICIENT_CYCLES` | Total cycles < GATE7_MIN_UPLINKS |
| 3 | `GATE_FAIL_INSUFFICIENT_UPLINKS` | Successful uplinks < GATE7_MIN_UPLINKS |
| 4 | `GATE_FAIL_CONSEC_MODBUS` | Consecutive Modbus failures > GATE7_MAX_CONSEC_FAILS |
| 5 | `GATE_FAIL_CONSEC_UPLINK` | Consecutive uplink failures > GATE7_MAX_CONSEC_FAILS |
| 6 | `GATE_FAIL_CYCLE_DURATION` | Cycle duration > GATE7_MAX_CYCLE_DURATION_MS |
...
[GATE7-R4631] Cycle 1 | modbus=OK | uplink=OK | dur=XXX ms | elapsed=30 s
[RUNTIME] Cycle 2: read=OK (2 regs), uplink=OK (port=10, 4 bytes)
[GATE7-R4631] Cycle 2 | modbus=OK | uplink=OK | dur=XXX ms | elapsed=60 s
...
[RUNTIME] Cycle 10: read=OK (2 regs), uplink=OK (port=10, 4 bytes)
[GATE7-R4631] Cycle 10 | modbus=OK | uplink=OK | dur=XXX ms | elapsed=300 s
```

### Summary Phase
```
[GATE7-R4631] STEP 3: Soak summary
[GATE7-R4631]   Duration          : 300XXX ms (300 s)
[GATE7-R4631]   Total Cycles      : 10
[GATE7-R4631]   Modbus OK / FAIL  : 10 / 0
[GATE7-R4631]   Uplink OK / FAIL  : 10 / 0
[GATE7-R4631]   Max Consec Fail   : modbus=0, uplink=0
[GATE7-R4631]   Max Cycle Duration: XXX ms
...
## Verification
1. Serial output shows `[GATE7-R4631] PASS` and `[GATE7-R4631] === GATE COMPLETE ===`
2. ChirpStack device events show at least 3 uplinks on port 10
3. No watchdog resets or HardFault during soak
